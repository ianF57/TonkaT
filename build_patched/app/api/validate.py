...
STALE_SECONDS_BY_CAT: dict[str, int] = {
    "crypto":      3_600,    # 60 min  — crypto is 24/7, short tolerance
    "stocks":      57_600,   # 16 hrs  — covers overnight + weekend gaps
    "forex":       57_600,   # 16 hrs  — markets close Fri 17:00 ET
    "commodities": 57_600,   # 16 hrs  — futures have settlement gaps
    "indices":     57_600,   # 16 hrs  — mirrors equity hours
}
...
# Yahoo Finance fetch
# ──────────────────────────────────────────────────────────────────────────────
async def _yahoo_spot(symbol: str, client: httpx.AsyncClient) -> dict | None:  # noqa: ARG001
    """Return {price, exchange_ts, exchange_raw} or None on failure.

    The ``client`` argument is retained for API compatibility but the real HTTP
    work is delegated to ``yahoo_chart_get`` which handles crumb auth, host
    fallback, User-Agent headers, and retries internally.
    """
    params = {"interval": "1m", "range": "1d", "includePrePost": "false"}
    try:
        data   = await yahoo_chart_get(symbol, params)
...
def _classify(asset: dict, spot: dict | None, now: float) -> dict:
    ...
    alert = None
    cat_stale_secs = STALE_SECONDS_BY_CAT.get(asset["cat"], 3_600)
    if spot["exchange_raw"] and (now - spot["exchange_raw"]) > cat_stale_secs:
...
async def _refresh_all() -> list[dict]:
    ...
    # _yahoo_spot now uses yahoo_chart_get internally; pass None as client placeholder
    spots = await asyncio.gather(*[_yahoo_spot(a["symbol"], None) for a in ASSETS])
...
async def validate_category(cat: str) -> dict:
    ...
    now    = time.time()
    spots  = await asyncio.gather(*[_yahoo_spot(a["symbol"], None) for a in subset])
...
async def validate_asset(asset_id: str) -> dict:
    ...
    now  = time.time()
    spot = await _yahoo_spot(asset["symbol"], None)
